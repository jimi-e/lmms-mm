...
class _QwenVLCaptioner:
    def __init__(self, model_name: str = 'Qwen/Qwen2.5-VL-7B-Instruct', device: str = 'cuda') -> None:
        """Load Qwen2.5-VL in a way that avoids CUDA segfaults.

        Strategy:
        - Prefer AutoModelForImageTextToText (Vision2Seq is deprecated and triggered segfaults here).
        - No device_map='auto' to avoid odd shard/device placements.
        - Honor FS_TORCH_DTYPE (bf16/fp16/fp32) and FS_ATTN_IMPL (eager/flash_attention_2/sdpa).
        - Set PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True to reduce frag.
        """
        import torch
        ...
        # attn impl
        attn_impl = (os.getenv('FS_ATTN_IMPL', 'eager') or 'eager').lower()
        if attn_impl == 'auto':
            attn_impl = 'eager'  # safer default on boxes without flash/xformers
        print(f"[DEBUG] Using attn_implementation: {attn_impl}")
        ...
        try:
            ...
            # model (no device_map to avoid segfaults seen with Vision2Seq+auto)
            print("[DEBUG] Loading model (ImageTextToText preferred)...")
            ...
            if torch.cuda.is_available():
                ...
                print(f"[DEBUG] GPU memory after model loading - Allocated: {allocated:.2f}GB, Reserved: {reserved:.2f}GB")
        except Exception as e:
...
def select_frames_by_framesearch(
    ...
    *,
    ...
    if not cache_status['caption']:
        # Build the captioner model BEFORE using decord to avoid CUDA/init conflicts
