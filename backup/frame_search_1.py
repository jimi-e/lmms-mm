# 8.20 14:00 能完整跑完所有 caption改了提升到100token。
import os
import json
import math
import hashlib
...
CONFIG: Dict[str, object] = {
    'PIPELINE_MODE': 'from_caption',     # full | caption_only | from_caption
    'USE_OLLAMA': True,          # True -> caption/reason 默认用 Ollama
    'USE_OPENAI': False,          # True -> score 默认用 OpenAI
    'CAPTION_BACKEND': 'ollama',      # 'hf' | 'ollama' | None (None=跟随 USE_OLLAMA)
...
class _QwenVLCaptioner:
    def __init__(self, model_name: str = 'Qwen/Qwen2-VL-7B-Instruct', device: str = 'cuda') -> None:
        ...
        try:
            ...
            self.model = AutoModelForCausalLM.from_pretrained(fb, trust_remote_code=True).to(device)
        self.device = device
...
class _TextReasoner:
    """Use Qwen2-VL as text-only chat LLM to produce reasoning lines in JSONL style."""
    def __init__(self, model_name: str = 'Qwen/Qwen2-VL-7B-Instruct', device: str = 'cuda') -> None:
        from transformers import AutoProcessor, AutoModelForCausalLM
        try:
            self.processor = AutoProcessor.from_pretrained(model_name, trust_remote_code=True)
