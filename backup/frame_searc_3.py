...
# Lazy-load decord to avoid potential CUDA/init conflicts when loading big VLMs on GPU.
...
def _load_decord() -> None:
    ...
    if VideoReader is not None and cpu is not None:
        return
    try:
        from decord import VideoReader as _VR, cpu as _cpu  # type: ignore
        VideoReader, cpu = _VR, _cpu
...
class _QwenVLCaptioner:
    def __init__(self, model_name: str = 'Qwen/Qwen2.5-VL-7B-Instruct', device: str = 'cuda') -> None:
        """Load Qwen2.5-VL with enhanced error handling and safety checks."""
        print(f"[DEBUG] Initializing _QwenVLCaptioner with model: {model_name}, device: {device}")
        
        # Import torch first and check environment
        try:
            import torch
            ...
            if torch.cuda.is_available():
                ...
                # Don't call current_device() here - it can cause segfault in certain contexts
                # Instead, check CUDA_VISIBLE_DEVICES
                cuda_visible = os.environ.get('CUDA_VISIBLE_DEVICES', 'all')
                print(f"[DEBUG] CUDA_VISIBLE_DEVICES: {cuda_visible}")
                
                # Try to get memory info without current_device()
                try:
                    # If device is specified as cuda:N, extract N
                    if ':' in str(device):
                    ...
                    print(f"[DEBUG] Initial GPU memory - Allocated: {allocated:.2f}GB, Reserved: {reserved:.2f}GB")
                except Exception as e:
        ...
        # torch dtype from env
        dtype_env = (os.getenv('FS_TORCH_DTYPE', 'bf16') or 'bf16').lower()
        if dtype_env == 'fp16':
        ...
        print(f"[DEBUG] Using torch_dtype: {torch_dtype}")
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                import gc
                gc.collect()
                print("[DEBUG] Garbage collection done")

            # Load processor
            print(f"[DEBUG] Loading processor for {model_name}...")
            ...
            # Load model with device_map for safer CUDA handling
            ...
            if device == "cpu":
                ...
                print("[DEBUG] Model loaded on CPU successfully")
            else:
                # Try using device_map="auto" for automatic placement
                try:
                    print("[DEBUG] Attempting to load model with device_map='auto'...")
                    self.model = AutoModelForVision2Seq.from_pretrained(
                        model_name,
                        trust_remote_code=True,
                    ...
                    print("[DEBUG] Model loaded with auto device mapping successfully")
                    
                    # Check where the model was placed
                    if hasattr(self.model, 'hf_device_map'):
                        print(f"[DEBUG] Device map: {self.model.hf_device_map}")
                    
                except Exception as e:
                    print(f"[DEBUG] Auto device mapping failed: {e}, falling back to manual placement")
                    
                    # Fallback: Load on CPU first, then move to CUDA manually
                    self.model = AutoModelForVision2Seq.from_pretrained(
                        model_name,
                        trust_remote_code=True,
                        torch_dtype=torch_dtype,
                    ...
                    print("[DEBUG] Model loaded on CPU, preparing for CUDA transfer...")
                    
                    # Clear cache before moving
                    torch.cuda.empty_cache()
                    gc.collect()
                    
                    # Try to move to CUDA with error handling
                    try:
                        # Move model in a more controlled way
                        ...
                        print(f"[ERROR] Failed to move model to CUDA: {cuda_err}")
                        print("[DEBUG] Keeping model on CPU as fallback")
            ...
            # Final memory check if on CUDA
            if torch.cuda.is_available() and self.device != "cpu":
                try:
                    allocated = torch.cuda.memory_allocated() / 1024**3
                    reserved = torch.cuda.memory_reserved() / 1024**3
                    print(f"[DEBUG] Final GPU memory - Allocated: {allocated:.2f}GB, Reserved: {reserved:.2f}GB")
                except Exception as e:
                    print(f"[DEBUG] Could not get final memory info: {e}")
            
            print("[DEBUG] _QwenVLCaptioner initialization complete")
                
        except Exception as e:
            print(f"[ERROR] Failed to initialize _QwenVLCaptioner: {e}")
            import traceback
            traceback.print_exc()
            raise
...
class _TextReasoner:
    ...
    def reason_chunk(self, items: List[Dict], question: str, max_new_tokens: int = 1024) -> Dict[int, str]:
        ...
        # Use proper chat template format for Qwen2.5-VL
        messages = [
            {'role': 'user', 'content': [{'type': 'text', 'text': prompt_text}]}
        ]
        text = self.processor.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
        
        # For text-only reasoning, don't pass images
...
def select_frames_by_framesearch(
    ...
    *,
    ...
    # Defer decord loading until after captioner creation to avoid CUDA/init conflicts.
    ...
    # We'll open the video after captioner creation.
    total = 0
    avg_fps = 0.0
    cand_idxs: List[int] = []
    ...
    if caps_json is None:
        # Try migrate from legacy per-run captions_*.json (stats will be filled later)
        caps_json = _migrate_old_caption_cache(vdir, cap_json_path, video_path, 0.0, 0)
    ...
    if caps_json is None:
        ...
        if os.path.exists(old_single):
            try:
                ...
                caps_json = None
    if caps_json is None:
        # Create a placeholder; we'll update avg_fps/total after opening the video.
        caps_json = _init_caps_json(video_path, 0.0, 0)
        _write_json_atomic(cap_json_path, caps_json)

    # Build the captioner model BEFORE any video/CUDA operations
    capper = None
    if not cache_status['caption'] and mode != 'from_caption':
        print("[DEBUG] Building captioner before any video operations...")
        capper = _build_captioner(caption_backend, device=device, hf_model=caption_model)
        print("[DEBUG] Captioner built successfully")

    # ensure metadata fresh
    changed_meta = False
    # Open video now (after HF model is potentially loaded) and compute candidates
    print("[DEBUG] Opening video file...")
    _load_decord()
    vr_stats = VideoReader(video_path, ctx=cpu(0), num_threads=1)
    total = len(vr_stats)
    avg_fps = float(vr_stats.get_avg_fps()) if hasattr(vr_stats, 'get_avg_fps') else 0.0
    ...
    # Ensure VideoReader is available for downstream stages if needed
    try:
        vr  # type: ignore # may exist from caption stage
    except Exception:
        _load_decord()
        vr = VideoReader(video_path, ctx=cpu(0), num_threads=1)
    caps = [
